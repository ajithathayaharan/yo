...
class YoApi(val rpcOps: CordaRPCOps) {
    ...
    fun yo(@QueryParam(value = "target") target: CordaX500Name?, @QueryParam(value = "msg") msg: String): Response {
        val (status, message) = try {
            // Is the 'target' valid?
            val toYo = rpcOps.wellKnownPartyFromX500Name(target!!) ?: throw Exception("Party not recognised.")
            // Start the flow.
            val flowHandle = rpcOps.startFlowDynamic(YoFlow::class.java, toYo, msg)
...
@InitiatingFlow
@StartableByRPC
class YoFlow(val target: Party, val msg: String) : FlowLogic<SignedTransaction>() {
...
data class YoState(val origin: Party,
                   val target: Party,
                   val msg: String) : ContractState, QueryableState {
    override val participants get() = listOf(target)
    override fun toString() = "${origin.name}: $msg"
    ...
    override fun generateMappedObject(schema: MappedSchema) = YoSchemaV1.PersistentYoState(
            origin.name.toString(), target.name.toString(), msg)
